jQuery(document).ready(function($){if(!("autofocus"in document.createElement("input")))$("#name").focus();$("input[title], select[title]").tooltip({position:"center right",effect:"fade",events:{input:"focus, blur",widget:"focus ,blur"}});$("#contact-form").validator({"offset":[0,1]}).submit(function(e){if(!e.isDefaultPrevented())$.post("process-form.php",$(this).serialize(),function(data){$(".form-wrapper h2").fadeOut("fast");$("#contact-form").fadeOut("fast",function(){$(".message").hide().html(data);
$(".message").show("fast")})});return false});if(ie8up())var triggers=$(".get-directions-button").overlay({mask:{color:"#fff",loadSpeed:200,opacity:1},fixed:false,closeOnClick:true,onLoad:function(){$("#contact-form").data("validator").reset();addressViaGeolocation();$(".directions-form").show("fast");thisMap=googleMap({streetViewControl:true,mapTypeControl:true,navigationControl:true,mapId:"map-big"})}});$("#travel-mode-input, #unit-input, #get-directions").live("click",function(){googleDirections.getDirections()});
function ie8up(){if(/MSIE (\d+\.\d+);/.test(navigator.userAgent)){var ieversion=new Number(RegExp.$1);if(ieversion>=8)return true;else if(ieversion>=7)return false;else if(ieversion>=6)return false;else if(ieversion>=5)return false}else return true}function addressViaGeolocation(){if(!!navigator.geolocation)if(geoLatLng==null)navigator.geolocation.getCurrentPosition(currentLocation,show_map_error);else $("#from-input").val("My Current Location");function currentLocation(loc){geoLatLng=loc.coords.latitude+
","+loc.coords.longitude;$("#from-input").val("My Current Location")}function show_map_error(){}}if(ie8up()){var googleMap=function(mapOptions){var geocoder,map,panorama,latlng;geocoder=new google.maps.Geocoder;var myOptions={zoom:15,mapTypeId:google.maps.MapTypeId.ROADMAP,streetViewControl:true,mapTypeControl:false,navigationControl:false,companyAddress:"Atlanta GA",mapId:"map"};var myOptions=$.extend({},myOptions,mapOptions);map=new google.maps.Map(document.getElementById(myOptions.mapId),myOptions);
geocoder.geocode({"address":myOptions.companyAddress},function(results,status){if(status==google.maps.GeocoderStatus.OK){latlng=results[0].geometry.location;map.setCenter(latlng);map.setZoom(16);var marker=new google.maps.Marker({map:map,position:latlng});map.setOptions()}});return{map:map}};thisMap=googleMap({companyAddress:companyAddress});var googleDirections={mapContainer:document.getElementById("map-big"),dirContainer:document.getElementById("dir-container"),fromInput:document.getElementById("from-input"),
toInput:document.getElementById("to-input"),travelModeInput:document.getElementById("travel-mode-input"),unitInput:document.getElementById("unit-input"),dirService:new google.maps.DirectionsService,dirRenderer:new google.maps.DirectionsRenderer,map:thisMap.map,showDirections:function(dirResult,dirStatus){if(dirStatus!=google.maps.DirectionsStatus.OK){alert("Unable to get directions. Please ensure you are using a valid starting address");return}$("#dir-container").show("fast");googleDirections.dirRenderer.setMap(thisMap.map);
googleDirections.dirRenderer.setPanel(googleDirections.dirContainer);googleDirections.dirRenderer.setDirections(dirResult)},getSelectedTravelMode:function(){var value=googleDirections.travelModeInput.options[googleDirections.travelModeInput.selectedIndex].value;if(value=="driving")value=google.maps.DirectionsTravelMode.DRIVING;else if(value=="bicycling")value=google.maps.DirectionsTravelMode.BICYCLING;else if(value=="walking")value=google.maps.DirectionsTravelMode.WALKING;else alert("Unsupported travel mode.");
return value},getSelectedUnitSystem:function(){return googleDirections.unitInput.options[googleDirections.unitInput.selectedIndex].value=="metric"?google.maps.DirectionsUnitSystem.METRIC:google.maps.DirectionsUnitSystem.IMPERIAL},getDirections:function(){var fromStr=googleDirections.fromInput.value;var toStr=googleDirections.toInput.value;var dirRequest={origin:fromStr=="My Current Location"?geoLatLng:fromStr,destination:toStr,travelMode:googleDirections.getSelectedTravelMode(),unitSystem:googleDirections.getSelectedUnitSystem(),
provideRouteAlternatives:true};googleDirections.dirService.route(dirRequest,googleDirections.showDirections)},init:function(){var latLng=new google.maps.LatLng(37.77493,-122.419415);googleDirections.map=new google.maps.Map(googleDirections.mapContainer,{zoom:13,center:latLng,mapTypeId:google.maps.MapTypeId.ROADMAP});googleDirections.getDirections()}}}});
